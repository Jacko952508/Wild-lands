...
function observe(gt){return gt.visible?{x:gt.x,y:gt.y}:null}
...
function tick(){const gt=hiddenWorld(state.episode,state.step),obs=observe(gt);
 if(pending&&obs&&state.phase!=="train"){const me=dist(pending.model,obs),be=dist(pending.baseline,obs),n=state.tests;state.modelError=(state.modelError*n+me)/(n+1);state.baselineError=(state.baselineError*n+be)/(n+1);state.tests++;if(me<be)state.wins++}
