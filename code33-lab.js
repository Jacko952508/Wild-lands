...
let S={phase:"train",episode:0,step:0,tickCount:0,lastTickAt:null,complete:false,frozenAt:null,trainSamples:0,cueStats:{0:{n:0,ax:0,ay:0},1:{n:0,ax:0,ay:0}},test:{n:0,m:0,b:0,g:[]},transfer:{n:0,m:0,b:0,g:[]},cueDetection:{n:0,correct:0}};
...
function predict(e){const h=H.filter(Boolean).slice(-2);if(h.length<2)return null;const a=h[0],b=h[1],dt=b.step-a.step,vx=(b.x-a.x)/dt,vy=(b.y-a.y)/dt,base={x:b.x+vx,y:b.y+vy};const c=S.lastCue;const k=(c===0||c===1)?learned[c]:{x:0,y:0};return{base,model:{x:base.x+k.x,y:base.y+k.y}}}
...
function report(){return{phase:S.phase,episode:S.episode,step:S.step,tickCount:S.tickCount,lastTickAt:S.lastTickAt,trainSamples:S.trainSamples,learnedCueEffects:learned,cueDetection:{tests:S.cueDetection.n,accuracy:S.cueDetection.n?S.cueDetection.correct/S.cueDetection.n:null},test:score(S.test),transfer:score(S.transfer),frozenAt:S.frozenAt,complete:S.complete}}
function tick(){if(S.complete)return;S.tickCount++;S.lastTickAt=new Date().toISOString();if(S.tickCount%220===0)console.log("V4_HEARTBEAT",JSON.stringify({phase:S.phase,episode:S.episode,step:S.step,tickCount:S.tickCount,lastTickAt:S.lastTickAt}));S.phase=phase(S.episode);const gt=W(S.episode,S.step),o=observe(render(gt,S.episode,S.step),S.step);if(o.cue!==null){S.lastCue=cueMeaning(o.cue,S.episode);if(S.phase!=="train"){S.cueDetection.n++;if(S.lastCue===gt.cue)S.cueDetection.correct++}}
...
if(!learned[0]||!learned[1])throw new Error("startup self-test failed: cue models");
tick();
