...
async function refreshBridgeTarget(){try{let r=await fetch("https://raw.githubusercontent.com/Jacko952508/Wild-lands/main/code33-web-target.txt",{headers:{"user-agent":"CODE33-WebBridge/1.1"},signal:AbortSignal.timeout(8000)});if(!r.ok)throw Error("raw-control "+r.status);let raw=(await r.text()).trim();if(raw){let t=await safeURL(raw);if(t!==webBridge.target){webBridge.target=t;webBridge.hash=null;webBridge.status="target-changed";console.log("WEB_BRIDGE_TARGET",JSON.stringify({target:t}))}else console.log("WEB_BRIDGE_CONTROL_OK",JSON.stringify({target:t}))}else console.log("WEB_BRIDGE_CONTROL_EMPTY")}catch(e){webBridge.error=String(e.message||e);console.log("WEB_BRIDGE_CONTROL_ERROR",JSON.stringify({error:webBridge.error}))}}
